...
- `AttributeKey` `:71`: BITRATE=0, DURATION=1, VBR=2, SAMPLE_RATE=4, BIT_DEPTH=5 (3 absent). `FileData.get_attribute_map()→dict[AttributeKey,int]` `:545`, silently drops unknown keys. Heuristic: BITRATE(±VBR)=lossy/mp3; SAMPLE_RATE+BIT_DEPTH=lossless/flac; many peers omit → treat sparse. `FileData.decode_attributes()→FileAttributes` `:558` = typed view over the map: `bit_rate, duration(=.length), is_vbr, sample_rate, bit_depth` (all Optional) + props `.is_lossless` (sample_rate&bit_depth set) / `.is_lossy` (bit_rate & !lossless).
...
- **Browse/download convenience** (client facade + transfers): `client.browse_user(username)→BrowseResult` (`commands.py PeerBrowseCommand` → `browse.py`) = navigable tree from the flat shares reply: `BrowseResult.get_directory(path)`/`.iter_files()`/`.iter_file_paths(include_locked=)`; `.root`=BrowseDirectory with `.get_path(subpath)`/`.walk()`/`.all_files()`/`.file_paths()`. `client.download_directory(username, directory, recursive=True, include_locked=False, paused=False)→list[Transfer]` browses then enqueues. `transfers.download_many(username, filenames, paused=False)→list[Transfer]` `manager.py:252` = per-file `download()`, skip+log failures. ⚠ **BROWSE filenames are LEAF-only** — full remote path = `dir.name + '\' + leaf` (use `file_paths()`/`download_directory`); SEARCH filenames are already full paths.
...
9. Peer browse can hang forever — `client.browse_user()` / low-level `execute(timeout=)` + catch asyncio.TimeoutError. `commands.py`. BROWSE `FileData.filename`=LEAF only (full path = `dir.name+'\'+leaf`, use `file_paths()`); SEARCH filenames are full paths.
...
# 3. browse peer (high-level navigable tree) + download a folder
br = await client.browse_user(username)            # BrowseResult; raises on no-reply/timeout
folder = br.get_directory(r'shared\Music\Album')   # BrowseDirectory | None
for f in folder.files:                             # NB: FileData.filename = LEAF name in browse
    a = f.decode_attributes()                      # FileAttributes: .bit_rate/.is_lossless/...
paths = list(folder.file_paths())                  # full remote paths (dir.name + '\' + leaf)
await client.transfers.download_many(username, paths)
await client.download_directory(username, r'shared\Music\Album')   # browse + enqueue all files
# low-level single dir (no reply ⇒ hang): execute(timeout=) + catch asyncio.TimeoutError
